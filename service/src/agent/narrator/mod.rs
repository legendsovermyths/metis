...
impl Agent for Narrator {
    fn generate(&mut self, _message: Option<String>) -> Result<AgentResponse> {
        let ts = self.context.lock().unwrap()
            .teaching_state.clone()
            .ok_or(MetisError::AgentError("No teaching state set".into()))?;

        let current_topic = ts.journey.journey
        ...
        let reference_material = Self::load_topic_content(&ts.journey.chapter_dir, &current_topic.name);
        ...
        let response = runtime().block_on(async {
            ...
            client.generate("Generate the next dialogue chunk.".to_string()).await
        })?;
...

        {
            ...
            ts.progress.push_dialogue(parsed.dialogue.clone(), parsed.topic_complete, &journey);
        ...
        Ok(AgentResponse::dialogue(parsed.dialogue))
    }
